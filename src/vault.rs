...
impl VaultApp {
    ...
    pub fn init(&self, force: bool) -> Result<()> {
        ...
        if self.path.exists() && !force {
            println!(
                "Vault file already exists at {}. Use --force to overwrite.",
                self.path.display()
            );
            return Ok(());
        }
    ...
    pub fn add(&self, name: &str) -> Result<()> {
        ...
        let salt_str = envelope
            ...
            .get("salt")
            .and_then(|s| s.as_str())
            .context("kdf.salt missing from envelope")?;
        let salt_b = base64::engine::general_purpose::STANDARD
            .decode(salt_str)
            .context("decoding base64 salt")?;
        ...
        let mut entries: Vec<Entry> =
            serde_json::from_slice(&pt).context("parsing decrypted vault JSON")?;
        // prompt for fields
        ...
        let password = if password_raw.trim().is_empty() {
            password_generator(16, true)
        } else {
            password_raw.trim().to_string()
        };
        let entry = Entry {
            name: name.to_string(),
        ...
        storage::write_envelope(&self.path, &envelope, self.no_backup)
            .context("writing vault envelope to disk")?;
